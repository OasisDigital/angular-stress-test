import { moduleBuildContents } from './template-module-bazel';
import { appModuleBuildContents } from './template-app-module-build';
...
class Component extends Writeable {
  ...
  private writeTs(deterministic: boolean) {
...
`;
  ...
  private writeHtml(deterministic: boolean) {
    let html = `<span>component {{myName}}<span *ngIf="parentName">,
  ...
  write(deterministic: boolean) {
    this.writeTs(deterministic);
    this.writeHtml(deterministic);
  }
}
...
`;
  ...
  writeBuild() {
    const sourcefileNames = [this.fileName + '.ts'];
    const assetfileNames: string[] = [];
    this.components.forEach(c => {
      sourcefileNames.push(c.fileName + '.ts')
      assetfileNames.push(c.fileName + '.html')
    });

    const build = moduleBuildContents(this.name, sourcefileNames, assetfileNames);
    mkdirp.sync(`${baseDir}${this.name}`);
    const buildFile = `${baseDir}${this.name}/BUILD.bazel`;
...
function generateModules(nModules: number, nComponents: number) {
  ...
  for (let m = 0; m < nModules; m++) {
    ...
    modules.push(module);
    const componentsInThisModule: Component[] = [];
...
`;
  ...
  modules.forEach(m => {
    ts += `import { ${m.name} } from './${m.path}/${m.fileName}';\n`;
  });
...
function generateAppModuleBuild() {
  const build = appModuleBuildContents(modules.map(m => m.name));
  const buildFile = `BUILD.bazel`;
  fs.writeFileSync(baseDir + buildFile, build);
}


export function generate(dir: string, nModules, nComponents, deterministic, bazel) {
  baseDir = dir + '/';

  // Top level module and component are separate, try to avoid off-by-one:
  ...
  if (bazel) {
    modules.forEach(m => m.writeBuild());
